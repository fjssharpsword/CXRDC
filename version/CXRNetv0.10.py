...
class CXRClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        #return x, fc_fea, out #return attention map
...
class ROIGenerator(object):
    ...
    def ROIGeneration(self, ori_image, fm_cuda, weight_softmax, label):
        ...
        for i in range(0, bz):
            ...
            class_idx = np.where(label[i]==1.0)[0] #label
            if len(class_idx)>0: #disease sample
                ...
                # crop from ori image 
                image = ori_image[i].numpy().reshape(config['TRAN_CROP'],config['TRAN_CROP'],3)
    ...
    def binImage(self, heatmap):
        #_, heatmap_bin = cv2.threshold(heatmap, 127, 255, cv2.THRESH_BINARY)#simple filter, threshold can be varied
        _, heatmap_bin = cv2.threshold(heatmap, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)#Otsu filter
        return heatmap_bin

    def selectMaxConnect(self, heatmap):
        #connectivity denotes maximum number of orthogonal hops to consider a pixel/voxel as a neighbor.
