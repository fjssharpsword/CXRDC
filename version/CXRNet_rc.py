...
class ROIGenerator(object):
    ...
    def ROIGeneration(self, ori_image, fm_cuda, weight_softmax, label):
        ...
        for i in range(0, bz):
            ...
            #======================================================================
            class_idx = np.where(label[i]==1)[0] #for training
            if len(class_idx)>0: 
                cam = weight_softmax[class_idx].dot(cam) #class activated map
            #cam = np.expand_dims(weight_softmax[label[i]], axis=0).dot(cam) #for testing
            #======================================================================
            cam = cam.sum(axis=0)
            cam = cam.reshape(h,w)
            cam = cam - np.min(cam)
            ...
            image = image[int(config['TRAN_CROP']*0.334):int(config['TRAN_CROP']*0.667),int(config['TRAN_CROP']*0.334):int(config['TRAN_CROP']*0.667),:]
